...
//.map() creates a new array, Number() turns strings into numbers
const nums = strNums.map(item => Number(item));

// Find the sum of the even values
// .filter() creates a new array only of values that pass a functin, in this case checking for even numbers.
// .reduce() then reduces the values of the array to a single value, adding them here
...
// since the numbers are in an array, they can be checked in .forEach() and sorted by their values
...
// .findIndex will get the index of the first instance of a value that matches a function, here value + index = 512
